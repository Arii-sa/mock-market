...
@section('title', '取引チャット（購入者）')

@section('css')
<link rel="stylesheet" href="{{ asset('css/transactions/buyer.css') }}">
...
<div class="transaction-container">
    ...
    {{-- ヘッダー --}}
    <div class="buyer-transaction-header">
        <div class="partner">
            <div class="partner-user">
                @php
                    $sellerProfile = $transaction->seller->profile;
                    $sellerImg = $sellerProfile && $sellerProfile->img_url
                        ? asset('storage/' . $sellerProfile->img_url)
                        : null;
                @endphp

                {{-- プロフィール画像 --}}
                @if($sellerImg)
                    <img class="partner-image" src="{{ $sellerImg }}" alt="" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                @endif
                    <div class="partner-image partner-image--default" style="{{ $sellerImg ? 'display:none;' : '' }}"></div>

                {{-- 名前 --}}
                <p class="partner-name">
                    「 {{ $transaction->seller->name }} 」さんとの取引画面
                </p>
            </div>
        </div>


        @if (auth()->id() === $transaction->buyer_id && $transaction->status === 'trading')
